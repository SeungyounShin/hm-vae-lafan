...
class Skeleton:
    def __init__(
        ...
        self._offsets = torch.Tensor(offsets).cuda().double()
        ...
        for i in range(self._offsets.shape[0]):
            if self._parents[i] == -1:
                ...
                positions_world.append(
                    qrot(rotations_world[self._parents[i]].double(), expanded_offsets[:, :, i].double())
...
if __name__ == "__main__":
    ...
    path = "/home/dataset/ubisoft-laforge-animation-dataset/output/BVH/dance2_subject3.bvh"
    ...
    ani = read_bvh(path)
    pose_axis_angle  = ani.quats
    trans = torch.tensor(ani.pos[:,0,:]).unsqueeze(0)  # [T x 3]
    pose_quat_angle = axis_angle_to_quaternion(torch.tensor(pose_axis_angle.copy())) # [T x 22 x 4]
    L = pose_quat_angle.shape[0]
    ...
    global_pos = lafan_skeleton.forward_kinematics(
        rotations = pose_quat_angle.unsqueeze(0).double(),
        root_positions = torch.zeros(1,L,3))
    
    root_mean = trans[0,:,:].mean(dim=-2)
    ...
    def update(frame):
        ax.cla()
        sample_pos  = global_pos[0,int(frame),:,:]
        ...
        for e in edges:
            ...
            ax.set_zlim([root_mean[1]-100,root_mean[1]+100]) 

    animm = FuncAnimation(fig, update, frames=[i for i in range(2500,3000,5)])

    animm.save('fk_layer_lafan.gif', writer='imagemagick')
