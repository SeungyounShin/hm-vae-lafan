import re, os, ntpath
import numpy as np
import utils.lafan_utils as utils
import torch
...
class Anim(object):
    ...
    def __init__(self, quats, pos, offsets, parents, bones, frametime):
        """
...
def read_bvh(filename, start=None, end=None, order=None):
    ...
    for line_num,line in enumerate(f):
        ...
        if dmatch:
            data_block = np.array(list(map(float, dmatch))) # 69
            N = len(parents)
    ...
    #[eudler to axis angles] rotation shape T, 22, 3 euler angle
    #rotmat = euler_angles_to_matrix(torch.tensor(rotations),"ZYZ") 
    #rotations = matrix_to_axis_angle(rotmat).numpy()

    #rotations = utils.euler_to_quat(np.radians(rotations), order=order) 
    rotations = euler_angles_to_matrix(torch.tensor(np.radians(rotations)),"ZYX")
    rotations = matrix_to_quaternion(torch.tensor(rotations.clone()))
    #rotations = utils.remove_quat_discontinuities(rotations)
    rotations = quaternion_to_axis_angle(rotations.clone()).numpy()
