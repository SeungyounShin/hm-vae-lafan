...
if __name__ == "__main__":
    ...
    path = "/home/dataset/lafan_for_hm_vae_fps30/aiming1_subject4.npy"
    ...
    np_data = np.load(path) # [T x 531]
    T = np_data.shape[0]
    global_pos = np_data[:,22*6+22*3*3:22*6+22*3*3+22*3] # [T x 66]
    global_pos = torch.tensor(global_pos).view(T,22,3)
    x_maen, y_mean, z_mean = global_pos.mean(dim=[0,1])
    ...
    def update(frame):
        ax.cla()
        sample_pos  = global_pos[int(frame),:,:]
        ...
        for e in edges:
            ...
            ax.set_xlim([x_maen-pad,x_maen+pad])
            ax.set_ylim([z_mean-pad,z_mean+pad]) 
            ax.set_zlim([y_mean-pad,y_mean+pad]) 

    animm = FuncAnimation(fig, update, frames=[i for i in range(2500,3000,5)])

    animm.save('fk_layer_lafan_np_check.gif', writer='imagemagick')
